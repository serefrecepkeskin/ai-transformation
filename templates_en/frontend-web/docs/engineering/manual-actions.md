...
- [ ] After adopting this template: make one test edit to a UI file and confirm
      **both** hooks fire on the runtime you use — formatting and the design
      check. A silent failure looks exactly like "no findings". Claude Code
      reads `.claude/settings.json`; Copilot reads `.github/hooks/`.
      TODO(confirm)
...
- [ ] Check whether the design hook runs at full fidelity in this repo: run
      `node .claude/skills/impeccable/scripts/detect.mjs <a UI file>` and look
