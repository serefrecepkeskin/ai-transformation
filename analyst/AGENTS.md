...
2. **No sensitive data in artifacts.** Column names, types and row *shapes* are
   ...
   strings live in `config/` and `.env*` — never read, never edited; the only
   config files you touch are the root templates `default.ini` and
   `env.example`, with placeholder values. Ask for a credential by *name*,
   never by contents. The deny rules in
...
Copilot. Installed by the company kit (`install.sh analyst <repo>`);
the kit's `bootstrap-prompt-greenfield.md` fills the catalog and connections.
