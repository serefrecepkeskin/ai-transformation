# Bootstrap prompt — paste this into the agent after installing the kit

You are setting up the AI-agent scaffolding that was just copied into this
repository (kit profile: `{{PROFILE}}`, AI tool: `{{TOOL}}`). The scaffolding
is generic; your job is to make it true for **this** codebase, from evidence only.

The install copied only the files of the tool in use: `claude` leaves out the
Copilot side (`.github/copilot-instructions.md`, `.github/agents/`,
`.github/hooks/`, `copilot-setup-steps.yml`, `.vscode/`), `copilot` leaves out
the Claude side (`CLAUDE.md`, `.claude/settings.json`, `.mcp.json`), `both`
keeps everything. Where `AGENTS.md`, `docs/` or a skill names a file of the
other runtime, drop that mention — never describe a guardrail that is not
installed.

If the repo turns out to be empty — no manifest, no source to read — this is the
wrong prompt: use `bootstrap-prompt-greenfield.md` (`./install.sh … --new`),
which decides the stack with the human and scaffolds it, instead of surveying
...
- `docs/domain/glossary.md` and `business-rules.md` — only terms you can point
  ...
  domain.
- frontend: `docs/screens.md` — every screen a user can reach, from the router
  and the menu source: route, gate, roles, expected states. `ui-check` walks
  this table; an empty one makes it toothless.

### If the repo already had an `AGENTS.md`

`install.sh` never overwrites — the repo's own guide was kept and the kit's
template was skipped. Its rules stay as they are; do not rewrite or duplicate
them. Add only what the kit needs, taken from
`{{PROFILE}}/AGENTS.md` in the kit: the **Capabilities (skills &
agents)** list, the **Guardrails** paragraph and the **Work trail** section,
adapted to the names this repo actually uses. Mention the doc taxonomy only if
this repo adopts it. Leave no `PLACEHOLDER` behind.

## 3. Tailor the skills

The stack-specific skills (`new-endpoint`, `db-migration`, `new-component`,
`ui-check`) ship with a PLACEHOLDER line. Rewrite each with this repo's real
...
Each profile brings its own ecosystem's runner, so there is no second
toolchain to install:

| Profile | Commit-time runner | Files it shipped | Wired by |
| --- | --- | --- | --- |
...
`install.sh` copied those files **only where the repo did not already
have them**. So first find out which case you are in, then do that case's work.
There must end up being exactly one definition of the gate.
...
- Python: add `[tool.ruff]` (and `[tool.pylint]` if you keep that hook) to the
  **existing** `pyproject.toml` — the kit deliberately ships no
...
- Frontend: `npm i -D husky lint-staged`, then
  ...
  not `vitest run`, fix the last line of `.husky/pre-commit`. Wire the e2e
  floor: `npm i -D @playwright/test && npx playwright install chromium`, a
  `"test:e2e": "playwright test"` script, `test-results/`, `playwright-report/`
  and `e2e/.auth/` in `.gitignore`, and the `TODO(confirm)` lines in
  `playwright.config.ts` / `e2e/auth.setup.ts` from the real dev server and
  login page. Run it once; it fails loudly while `docs/screens.md` is empty.
- Then run the whole-tree sweep **once** — `pre-commit run --all-files`, or
  `npm run lint` — and read all of it. Existing code will fail; that is a
  finding to report, not something to fix in this pass. Report the counts per
  hook. Do **not** mass-reformat the repo, and do **not** loosen a rule to get
  the count to zero — a rule that genuinely does not fit is an ADR, not a config edit.
...
config in `package.json`, simple-git-hooks, a `.pre-commit-config.yaml` in a
JS repo, or a hand-written `.git/hooks/pre-commit`. Two gates is worse than one.
Merge the shipped checks into theirs and delete the kit's copy, or replace
theirs — pick one, say why, and make sure only one hook ends up running.
...
- Secrets layout. The deny rules close `config/**` and every `.env*`; the
  agent's only config files are the root templates `default.ini` and
  `env.example`. Check this repo against that and fix what does not fit:
  the template is named `.env.example` → rename it to `env.example` and update
  the pointers (a leading dot matches the `.env.*` deny, and deny rules cannot
  be excepted); `config/` holds **source code** here → narrow the two
  `config/**` rules to the credential-bearing files (`config/*.ini`,
  `config/*.yaml`, …) in both settings files and say so; the config template
  lives under `config/` → `TODO(confirm)` for a human: move it to the root as
  `default.ini`, or accept that the agent edits it blind.
- Copilot in use: `ls .github/agents/` shows `code-reviewer.agent.md` and
  `security-reviewer.agent.md`; if not, run `python3 <kit>/sync-copilot-agents.py .`.
  Claude Code in use: `.claude/settings.json` is present and its hook paths
  resolve.
- `agent-work/**/shots/` is in `.gitignore` (install adds it; confirm) — the
  rest of `agent-work/` travels with feature branches on purpose.
- frontend: `tsconfig.json` `exclude` covers `.claude/`, `.impeccable/` and
  `agent-work/`, and `.prettierignore` (shipped) is honored by the format
  script — otherwise the whole-tree checks trip over the vendored skill.
- CI: does a workflow already run these gates? The kit ships no CI file.
...
Do not touch application code. Do not create `agent-work/` — bootstrapping is
not a task. Do not commit. Do not open a PR.
