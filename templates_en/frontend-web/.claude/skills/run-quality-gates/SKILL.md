...
The commit hook (`.husky/pre-commit`) runs eslint over the staged files and
the suite. It is not a substitute for the gates below: `npx lint-staged` sees
only what is staged, so `npm run lint` over the whole tree is still gate 2.
`eslint --fix` rewrites files and then reports failure — that is a pass in two
steps: read what changed, keep the fix if it is right, run it again until clean.
Never `--no-verify` a commit; a hook that is wrong is a decision
(`record-decision`), not a flag.
