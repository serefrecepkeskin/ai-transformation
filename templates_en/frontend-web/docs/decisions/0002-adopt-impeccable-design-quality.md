...
- `.claude/skills/impeccable/` — the skill and its deterministic detector; both
  Claude Code and VS Code Copilot read that directory natively.
- `.claude/settings.json` and `.github/hooks/impeccable.json` — the same
  post-edit check, wired up once per runtime.
...
The detector is deterministic Node — no LLM, no API key — so it can also run in
CI (`npx impeccable@3 detect .`). Add it to this repo's own CI in **report-only**
mode first; making it blocking is a separate, later decision, taken once the
finding rate on this repo is known.
...
- Two hooks now coexist (formatting and design). They are separate entries and
  separate scripts, and each runtime wires them up in its own file — verify both
  actually fire after adopting the template, on the runtime you use; see
  [manual-actions.md](../engineering/manual-actions.md).
- The hook runs the **full** detector on every edit. Expect it to be slower
  than the format hook.
...
- **The vendored copy can run degraded.** The payload ships no `node_modules`,
  ...
  as a pass. Either add those four as devDependencies, or run the CLI
  (`npx impeccable@3 detect`) in CI, which brings its own parsers. Verify the
  hook's fidelity once after adopting — see
  [manual-actions.md](../engineering/manual-actions.md).
...
- Vendored upstream files are kept **verbatim** (except the install path, which
  points at `.claude/skills/`) and are exempt from the template's line and
  description budgets. Update them with `npx impeccable update`, never by hand.
