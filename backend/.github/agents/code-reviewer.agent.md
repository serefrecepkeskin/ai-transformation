...
---
<!-- GENERATED from .claude/agents/code-reviewer.md by sync-copilot-agents.py — do not edit -->
...
Read `AGENTS.md` (its project rules are the checklist that already caught real
bugs here), then `docs/engineering/conventions.md` and
`docs/engineering/architecture.md`, then the diff you were pointed at —
with enough surrounding code to confirm each finding against a call site or a
test. Keep going after the first issue.
...
make things up. Pre-existing problems go once under "residual risk". End with
a 2–3 sentence verdict naming test gaps and residual risk.
