...
PLACEHOLDER: one paragraph — what this service does, its consumers, the stack
(.NET version, ASP.NET Core / worker), and how it is deployed.
...
3. **Not done until the gates are green.** Run the `run-quality-gates` skill
   (build, format, tests) before any commit/PR.
...
8. **Validate at the boundaries.** External input (HTTP, queue, third-party
   API) is untrusted: validate at the edge; typed models inside. Respect
   nullable reference types — no `!` to silence warnings.
...
11. **Schema changes only via EF migrations** — never edit the database or a
    generated migration by hand; use the `ef-migration` skill.

## Commands

PLACEHOLDER — bootstrap-research fills this from the solution/csproj/CI.

- `dotnet run --project TODO` — run locally
...
- **skill `run-quality-gates`** — build + format + tests, report results.
...
- **skill `ef-migration`** — create/apply an EF Core schema migration safely.
...
- **agent `code-reviewer`** — priority-ordered diff audit
  (correctness → nullability/async → boundaries → tests).

Guardrails: a Copilot hook verifies formatting and reminds on docs drift at
session end (`.github/hooks/`); safe commands are pre-approved in
`.vscode/settings.json`. Details in [conventions.md](docs/engineering/conventions.md).
