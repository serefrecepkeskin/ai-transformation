---
name: implement-task
description: Implements a request end to end — understand, build, test, verify, self-review, open a PR. Use when given a task file, a ticket, or a plain feature/bugfix request that is bigger than a one-line edit. Stops and asks when the goal cannot be made verifiable.
---
...
If the work needs more than one session or touches more than a handful of
files, stop here and use `plan-feature` instead.
...
Smallest diff that satisfies the goal. Follow the conventions. New behavior
...
Run the proof from step 1 and read its output. Endpoint changes: call the
endpoint locally and paste the status + body. Logic changes: the test output.
Quote what you actually saw; "should work" is not a result.
