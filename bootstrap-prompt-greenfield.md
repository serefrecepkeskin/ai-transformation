...
Paste this into the agent after installing the kit into an **empty** repo.

You are setting up the AI-agent scaffolding that was just copied into this
repository (kit profile: `{{PROFILE}}`, AI tool: `{{TOOL}}` — only that
runtime's files were copied; mention no other). This repository is new: there
is little or no code to read yet. The scaffolding is generic; your job is to turn it into
...
- **frontend** — also settle the design direction for `docs/DESIGN.md`
  before any UI exists. The `impeccable` skill audits against that file, so an
  empty one makes it toothless. Start `docs/screens.md` with the first slice's
  screen; `ui-check` walks that table.
- **analyst** — there is no stack to choose. Ask instead which DB
...
- lint / format / type-check configured with the tools that were chosen — the
  kit ships the commit-time gate for its own ecosystem —
  `.pre-commit-config.yaml` + `requirements-dev.txt` (backend, pip) or
...
`backend`, **0003** for `frontend` (0001 is the AI-workflow ADR, and
...
`ui-check`) ship with a PLACEHOLDER line. Rewrite each against the paths and
...
- Copilot in use: `ls .github/agents/` shows the generated `*.agent.md` twins.
  Claude Code in use: `.claude/settings.json` hook paths resolve.
  `agent-work/**/shots/` is in `.gitignore`; frontend `tsconfig.json` excludes
  `.claude/`, `.impeccable/` and `agent-work/`.
...
Do not create `agent-work/` — bootstrapping is not a task. Do not commit and do
not open a PR — the human reviews first, then the `commit-and-pr` skill.
